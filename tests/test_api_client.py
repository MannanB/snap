...
class TestApiClass(unittest.TestCase):
    ...
    def test_api_key_rotation_calls(self):
        api = snap.API(key=['sample_key1', 'sample_key2'],
                       rotate_key=True,
                       rotate_method=snap.ROTATE_METHOD_CALLS,
                       rotate_max_calls=5,
                       key_method=snap.PARAMS, use_cache=False)
        api.add_endpoint('http://127.0.0.1:5000/Simple', name='simple')
    ...
    def test_api_key_rotation_time(self):
        api = snap.API(key=['sample_key1', 'sample_key2'],
                       rotate_key=True,
                       rotate_method=snap.ROTATE_METHOD_TIME,
                       rotate_max_time=30,
                       key_method=snap.PARAMS, use_cache=False)
        api.add_endpoint('http://127.0.0.1:5000/Simple', name='simple')
    ...
    def test_params_headers(self):
        api_headers = snap.API(key='sample_key1',
                               key_method=snap.HEADERS)
        ...
        api_params = snap.API(key='sample_key1',
                              key_method=snap.PARAMS)
    ...
    def test_cache(self):
        api = snap.API(key='sample_key1',
                       key_method=snap.PARAMS,
                       use_cache=True)
        resp1 = api.request_endpoint(endpoint=snap.Endpoint('http://127.0.0.1:5000/Simple'))
        resp2 = api.request_endpoint(endpoint=snap.Endpoint('http://127.0.0.1:5000/Simple'))
    ...
    def test_errors(self):
        api = snap.API(key='sample_key1',
                       ...
                       use_cache=True)
        try:
            api.request_endpoints(endpoints=snap.Endpoint('http://127.0.0.1:5000/Simple'), params={'abc': 123},
                                  headers={'def': 456})
        except ValueError:
            pass

        try:
            api.request_endpoints(endpoints=snap.Endpoint('http://127.0.0.1:5000/Simple'),
