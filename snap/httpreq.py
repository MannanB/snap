...
class Session:
    ...
    def request_bulk(self, method: str,
        ...
        :param num_threads: Amount of threads to be used
        ...
        if use_threads:
            return self.request_bulk_threaded(method, urls, params=params, headers=headers, data=data,
    ...
    def __thread_worker(self, method, max_conns, urls, params, headers, data, retries, retry_delay, per, use_async, ssl,
                        resp_q):
        if use_async:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            session = aiohttp.ClientSession(headers=self.global_headers, loop=loop)
            resps = loop.run_until_complete(
                self.request_bulk_async(method, urls, params, headers, session=session, max_connections=max_conns,
                                        per=per, retries=retries, retry_delay=retry_delay, ssl=ssl))
            loop.run_until_complete(session.close())
        else:
            session = requests.session()
            resps = self.request_bulk_sync(method, urls, params, headers, data, session=session, retries=retries,
                                           retry_delay=retry_delay, ssl=ssl)
            session.close()

        for i in range(len(urls)):
            resp_q.put((i, resps[i]))

    def request_bulk_threaded(self, method: str,
                              urls: list[str],
                              params: Optional[list[dict]] = None,
                              headers: Optional[list[dict]] = None,
                              data: Optional[list[dict]] = None,
        ...
        for i in range(num_threads):
            max_conns_i = max_connections // num_threads + int(max_connections % num_threads >= i + 1)
            max_urls_i = len(urls) // num_threads + int(len(urls) % num_threads >= i + 1)
            urls_i = urls[last_url_idx:last_url_idx + max_urls_i]
            params_i = None
            headers_i = None
            data_i = None
            if params:
                params_i = params[last_url_idx:last_url_idx + max_urls_i]
            if headers:
                headers_i = headers[last_url_idx:last_url_idx + max_urls_i]
            if data:
                data_i = data[last_url_idx:last_url_idx + max_urls_i]
            last_url_idx = last_url_idx + max_urls_i
            threads.append(threading.Thread(target=self.__thread_worker,
                                            args=(method, max_conns_i, urls_i, params_i, headers_i, data_i, retries,
                                                  retry_delay, per, use_async, ssl, resp_q,),
                                            daemon=True))
            threads[-1].start()

        for thread in threads:
            thread.join()
        resps = [None for _ in range(len(urls))]
    ...
    def get_bulk(self,
        ...
        return self.request_bulk(METHOD_GET, urls, params=params, headers=headers, session=session,
                                 ...
                                 num_threads=num_threads, max_connections=max_connections, per=per, retries=retries,
                                 retry_delay=retry_delay,
                                 use_async=use_async, ssl=ssl)
    ...
    def post_bulk(self,
        ...
        return self.request_bulk(METHOD_POST, urls, params=params, headers=headers, data=data, session=session,
                                 use_threads=use_threads, num_threads=num_threads, max_connections=max_connections,
